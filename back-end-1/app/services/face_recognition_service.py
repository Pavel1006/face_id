...
# Setting up logging to capture debug information
...
class FaceRecognizer:
    ...
    def recognize_face(self, image):
        try:
            ...
            faces = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml").detectMultiScale(image)
            if len(faces) == 0:
            ...
            logging.debug("Face resized and flattened.")

            face_pca = self.pca_service.transform(face_flattened)
            face_fisher = self.lda_service.transform(face_pca)

            # Check shapes
            logging.debug(f"Shape of fisherfaces: {self.fisherfaces.shape}")
            logging.debug(f"Shape of face_fisher: {face_fisher.shape}")

            # Handle edge cases
            if face_fisher is None:
                logging.error("LDA transformation returned None or invalid values.")
                raise ValueError("LDA transformation failed")

            distances = np.linalg.norm(face_fisher - self.fisherfaces, axis=1)
            label_idx = np.argmin(distances)

            # Log the detected face label
            logging.debug(f"Face recognized as label: {label_idx}, name: {self.label_map[str(label_idx)]}")
            return self.label_map[str(label_idx)]
